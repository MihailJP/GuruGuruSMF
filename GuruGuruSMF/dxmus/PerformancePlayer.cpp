...
namespace GuruGuruSmf {	namespace Dxmus	{
	...
	// 初期化
	int PerformancePlayer::Open(HWND handle, int channels)
	{
		Close();
	...
	// 演奏！
	int PerformancePlayer::Play(SmfData* smf, int option, int seek)
	{
		if(!isOpen) return -1;
		Stop();
		...
		if(smf->StoredType == StoredType::File){	// ファイルから読み込み
			hr = loader->LoadObjectFromFile(CLSID_DirectMusicSegment,
						IID_IDirectMusicSegment8,
						smf->FileName, (LPVOID*)&segment);
		}else{	// メモリから読み込み
		...
		bool isSkipBegin = ((option & PlayOption::SkipBegin) != 0);
		bool isSkipEnd = ((option & PlayOption::SkipEnd) != 0);
